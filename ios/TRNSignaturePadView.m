...
@implementation TRNSignatureView {
...
- (void)layoutSubviews
{
    ...
    if (!_loaded) {
        ...
        sign = [[TRNSignaturePadCanvasView alloc]
...
-(void) saveImage {
    ...
    if (!isSuccess) {
        NSLog(@"Error: Could not store signature file.");
        return;
    }
    
    NSString *base64Encoded = [imageData base64EncodedStringWithOptions:0];
    NSDictionary* result = @{ @"pathName": tempPath, @"encoded": base64Encoded };
    
    [self.manager emitSaved: result];
}
